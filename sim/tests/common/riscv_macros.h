...
//-----------------------------------------------------------------------
// 202: Помещаем в a1 адрес сегмента памятм, в который необходимо делать вывод (взят из файла sc_print.c)
// 203: Помещаем в a2 адрес метки test_ok
// 204: Побайтового начинаем записывать все байты строки
// 205-207: Проверка, что строка нуль-терминированная, получение \0 будет являться условием завершения и выхода из цикла
// Для RVTEST_FAIL всё по аналогии, но со строкой test_fail
//-----------------------------------------------------------------------
...
#define TEST_DATA /* Из-за того, что макросы RVTEST_PASS|FAIL находятся в середине программы, строки невыровненной длины вызывают смещение, поэтому лучше объявлять их в конце, т.е. в макросе TEST_DATA*/
        test_ok: .asciz "V\n"; /* Метка, содержащая значение вывода RVTEST_PASS*/
        test_fail: .asciz "DEAD\n"; /* Метка, содержащая значение вывода RVTEST_FAIL*/
        .align 9 /* Добавить выравнивание*/
